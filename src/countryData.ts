...
const allCountriesTemp = rawAllCountries.map((country) => {
  ...
  const areaItems: Country[] = (areaCodes ?? []).map((areaCode) => {
    const areaItem = {
      ...countryItem,
      regions,
  ...
  addCountryCode(countryItem.iso2, countryItem.dialCode, priority);
